...
<script>
    $(function () {
        ...
        $('#btnAdd').click(function () {
            ...
            $('#myForm').attr('action', '<?php echo base_url() ?>private/PortfoliosController/add');
        ...
        $('#btnSave').click(function () {
            // ENCAPSULANDO INPUTS
            var titulo      = $('input[name=title_portfolio]');
            var texto       = $('input[name=text_portfolio]');
            var link        = $('input[name=link_portfolio]');
            var imagem      = $('input[name=image_portfolio]');
            var extension   = $('input[name=image_portfolio]').val().split('.').pop().toLowerCase();
            ...
            if (link.val() === '') {
                link.parent().parent().addClass('has-error');
            } else {
                link.parent().parent().removeClass('has-error');
                result += '3';
            }
            if (imagem.val() !== '' && jQuery.inArray(extension, ['gif', 'png', 'jpg', 'jpeg']) === -1)
            {
                ...
                $('input[name=image_portfolio]').val('');
                ...
                $('#myForm').ajaxSubmit({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            sucesso('Portfolio ' + type + ' com sucesso', '');
                            showAllPortfolios();
                        } else {
        ...
        $('#showdata').on('click', '.item-edit', function () {
            var id_portfolio = $(this).attr('data');
            $('#myModal').modal('show');
            $('#img_atual').fadeIn('fast');
            $('#myModal').find('.modal-title').text('Editar');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/PortfoliosController/update');
            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>private/PortfoliosController/getById',
                data: {id_portfolio: id_portfolio},
                async: false,
                dataType: 'json',
                success: function (data) {
                    $('input[name=id_portfolio]').val(data.id_portfolio);
                    $('input[name=title_portfolio]').val(data.title_portfolio);
                    $('input[name=text_portfolio]').val(data.text_portfolio);
                    $('input[name=link_portfolio]').val(data.link_portfolio);
                    $('.aqui').attr("src", "<?php echo base_url('assets/uploads/portfolios/'); ?>" + data.image_portfolio);
                    if (data.status == '1') {
                        $('.ios-switch').removeClass('off').addClass('on').next('input:checkbox').attr('checked', true).val('1');
                        $('.stt').html('Ativo');
                    } else {
        ...
        $('input[name=image_portfolio]').on('change', function ()
        {
        ...
        $('#showdata').on('click', '.item-delete', function () {
            ...
            $('#btnDelete').unbind().click(function () {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>private/PortfoliosController/delete',
                    data: {id_portfolio: id_portfolio},
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao deletar', 'Item excluído!');
                            showAllPortfolios();
                        } else {
        ...
        $('#myForm2').on('click', '.jq_btn_excluir_marcados', function () {
            ...
            $('#btnDelete').unbind().click(function () {
                $.ajax({
                    ...
                    method: 'post',
                    async: false,
                    url: '<?php echo base_url() ?>private/PortfoliosController/deleteSelected',
                    data: data,
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao excluir', 'Itens excluídos!');
                            showAllPortfolios();
                        } else {
        ...
        function showAllPortfolios() {
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>private/PortfoliosController/getAll',
                async: false,
                dataType: 'json',
                success: function (data) {
                    var html = '';
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                                '<td class="center">' +
                                '<label class="pos-rel">' +
                                '<input name="excluir_todos[]" value="' + data[i].id_portfolio + '" type="checkbox" class="ace" />' +
                                '<span class="lbl"></span>' +
                                '</label>' +
                                '</td>' +
                                '<td>' + data[i].id_portfolio + '</td>' +
                                '<td style="width: 20%">' +
                                '<img src="<?php echo base_url('assets/uploads/portfolios/'); ?>' + data[i].image_portfolio + '" class="img-responsive" alt="User Image" width="250" height="">' +
                                '</td>' +
                                '<td>' + data[i].title_portfolio + '</td>' +
                                '<td>' + data[i].text_portfolio + '</td>' +
                                '<td>' + data[i].link_portfolio + '</td>' +
                                ...
                                '<i class="ace-icon fa fa-pencil bigger-130 item-edit" data="' + data[i].id_portfolio + '" title="Atualizar"></i>' +
                                '</a>' +
                                '<a class="red" href="javascript:;" >' +
                                '<i class="ace-icon fa fa-trash-o bigger-130 item-delete" data="' + data[i].id_portfolio + '" title="Remover"></i>' +
                                '</a>' +
                                '</div>' +
                                '</td>' +
                                '</tr>';
