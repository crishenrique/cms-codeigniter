...
<script>
    $(function () {
        ...
        $('#btnAdd').click(function () {
            ...
            $('#myForm').attr('action', '<?php echo base_url() ?>private/AboutController/add');
        ...
        $('#btnSave').click(function () {
            // ENCAPSULANDO INPUTS
            var titulo      = $('input[name=title_about]');
            var texto       = $('input[name=text_about]');
            var link        = $('input[name=link_about]');
            var imagem      = $('input[name=image_about]');
            var extension   = $('input[name=image_about]').val().split('.').pop().toLowerCase();
                ...
                $('#myForm').ajaxSubmit({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            sucesso('About ' + type + ' com sucesso', '');
                            showAllAbout();
                        } else {
        ...
        $('#showdata').on('click', '.item-edit', function () {
            var id_about = $(this).attr('data');
            $('#myModal').modal('show');
            $('#img_atual').fadeIn('fast');
            $('#myModal').find('.modal-title').text('Editar');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/AboutController/update');
            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>private/AboutController/getById',
                data: {id_about: id_about},
                async: false,
                dataType: 'json',
                success: function (data) {
                    $('input[name=id_about]').val(data.id_about);
                    $('input[name=title_about]').val(data.title_about);
                    $('input[name=text_about]').val(data.text_about);
                    $('input[name=link_about]').val(data.link_about);
                    $('.aqui').attr("src", "<?php echo base_url('assets/uploads/about/'); ?>" + data.image_about);
                    if (data.status == '1') {
                        $('.ios-switch').removeClass('off').addClass('on').next('input:checkbox').attr('checked', true).val('1');
                        $('.stt').html('Ativo');
                    } else {
        ...
        $('input[name=image_about]').on('change', function ()
        {
        ...
        $('#showdata').on('click', '.item-delete', function () {
            ...
            $('#btnDelete').unbind().click(function () {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>private/AboutController/delete',
                    data: {id_about: id_about},
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao deletar', 'excluído!');
                            showAllAbout();
                        } else {
        ...
        $('#myForm2').on('click', '.jq_btn_excluir_marcados', function () {
            ...
            $('#btnDelete').unbind().click(function () {
                $.ajax({
                    ...
                    method: 'post',
                    async: false,
                    url: '<?php echo base_url() ?>private/AboutController/deleteSelected',
                    data: data,
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao deletar', 'excluídos!');
                            showAllAbout();
                        } else {
        ...
        function showAllAbout() {
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>private/AboutController/getAll',
                async: false,
                dataType: 'json',
                success: function (data) {
                    var html = '';
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                                '<td class="center">' +
                                '<label class="pos-rel">' +
                                '<input name="excluir_todos[]" value="' + data[i].id_about + '" type="checkbox" class="ace" />' +
                                '<span class="lbl"></span>' +
                                '</label>' +
                                '</td>' +
                                '<td>' + data[i].id_about + '</td>' +
                                '<td style="width: 20%">' +
                                '<img src="<?php echo base_url('assets/uploads/about/'); ?>' + data[i].image_about + '" class="img-responsive" alt="User Image" width="250" height="">' +
                                '</td>' +
                                '<td>' + data[i].title_about + '</td>' +
                                '<td>' + data[i].text_about + '</td>' +
                                ...
                                '<i class="ace-icon fa fa-pencil bigger-130 item-edit" data="' + data[i].id_about + '" title="Atualizar"></i>' +
                                '</a>' +
                                '<a class="red" href="javascript:;" >' +
                                '<i class="ace-icon fa fa-trash-o bigger-130 item-delete" data="' + data[i].id_about + '" title="Remover"></i>' +
                                '</a>' +
                                '</div>' +
                                '</td>' +
                                '</tr>';
