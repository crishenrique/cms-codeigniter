...
<script>
    $(function () {
        ...
        $('#btnAdd').click(function () {
            ...
            $('#myForm').attr('action', '<?php echo base_url() ?>private/TeamsController/add');
        ...
        $('#btnSave').click(function () {
            // ENCAPSULANDO INPUTS
            var name_team   = $('input[name=name_team]');
            var office_team = $('input[name=office_team]');
            var link_team   = $('input[name=link_team]');
            var image_team  = $('input[name=image_team]');
            var extension   = $('input[name=image_team]').val().split('.').pop().toLowerCase();
            var url = $('#myForm').attr('action');
            var data = $('#myForm').serialize();
            var result = '';
            if (name_team.val() === '') {
                name_team.parent().parent().addClass('has-error');
            } else {
                name_team.parent().parent().removeClass('has-error');
                result += '1';
            }
            if (office_team.val() === '') {
                office_team.parent().parent().addClass('has-error');
            } else {
                office_team.parent().parent().removeClass('has-error');
                result += '2';
            }
            if (link_team.val() === '') {
                link_team.parent().parent().addClass('has-error');
            } else {
                link_team.parent().parent().removeClass('has-error');
                result += '3';
            }
            if (image_team.val() !== '' && jQuery.inArray(extension, ['gif', 'png', 'jpg', 'jpeg']) === -1)
            {
                ...
                $('input[name=image_team]').val('');
                image_team.parent().parent().addClass('has-error');
                return false;
            } else {
                image_team.parent().parent().removeClass('has-error');
                result += '4';
            }
            if (result === '1234') {
                $('#myForm').ajaxSubmit({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            sucesso('Integrante ' + type + ' com sucesso', '');
                            ShowAllTeams();
                        } else {
        ...
        $('#showdata').on('click', '.item-edit', function () {
            var id_team = $(this).attr('data');
            $('#myModal').modal('show');
            $('#img_atual').fadeIn('fast');
            $('#myModal').find('.modal-title').text('Editar');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/TeamsController/update');
            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>private/TeamsController/getById',
                data: {id_team: id_team},
                async: false,
                dataType: 'json',
                success: function (data) {
                    $('input[name=id_team]').val(data.id_team);
                    $('input[name=name_team]').val(data.name_team);
                    $('input[name=office_team]').val(data.office_team);
                    $('input[name=link_team]').val(data.link_team);
                    $('.aqui').attr("src", "<?php echo base_url('assets/uploads/teams/'); ?>" + data.image_team);
                    if (data.status == '1') {
                        $('.ios-switch').removeClass('off').addClass('on').next('input:checkbox').attr('checked', true).val('1');
                        $('.stt').html('Ativo');
                    } else {
        ...
        $('input[name=image_team]').on('change', function ()
        {
        ...
        $('#showdata').on('click', '.item-delete', function () {
            ...
            $('#btnDelete').unbind().click(function () {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>private/TeamsController/delete',
                    data: {image_team: image_team},
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao excluir', 'Excluído!');
                            ShowAllTeams();
                        } else {
        ...
        $('#myForm2').on('click', '.jq_btn_excluir_marcados', function () {
            ...
            $('#btnDelete').unbind().click(function () {
                $.ajax({
                    ...
                    method: 'post',
                    async: false,
                    url: '<?php echo base_url() ?>private/TeamsController/deleteSelected',
                    data: data,
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao excluir', 'Excluídos!');
                            ShowAllTeams();
                        } else {
        ...
        function ShowAllTeams() {
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>private/TeamsController/getAll',
                async: false,
                dataType: 'json',
                success: function (data) {
                    var html = '';
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                                '<td class="center">' +
                                '<label class="pos-rel">' +
                                '<input name="excluir_todos[]" value="' + data[i].id_team + '" type="checkbox" class="ace" />' +
                                '<span class="lbl"></span>' +
                                '</label>' +
                                '</td>' +
                                '<td>' + data[i].id_team + '</td>' +
                                '<td style="width: 20%">' +
                                '<img src="<?php echo base_url('assets/uploads/teams/'); ?>' + data[i].image_team + '" class="img-responsive" alt="User Image" width="250" height="">' +
                                '</td>' +
                                '<td>' + data[i].name_team + '</td>' +
                                '<td>' + data[i].office_team + '</td>' +
                                '<td>' + data[i].link_team + '</td>' +
                                ...
                                '<i class="ace-icon fa fa-pencil bigger-130 item-edit" data="' + data[i].id_team + '" title="Atualizar"></i>' +
                                '</a>' +
                                '<a class="red" href="javascript:;" >' +
                                '<i class="ace-icon fa fa-trash-o bigger-130 item-delete" data="' + data[i].id_team + '" title="Remover"></i>' +
                                '</a>' +
                                '</div>' +
                                '</td>' +
                                '</tr>';
