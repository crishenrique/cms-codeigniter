...
<script>
    $(function() 
    {
        ...
        $('#btnAdd').click(function() 
        {
            $('#myModal').modal('show');
            $('#img_atual').hide();
            $('#myModal').find('.modal-title').text('Add Novo Usuario');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/UsersController/add');
        ...
        {
            /* ENCAPSULANDO INPUTS */
            var nome = $('input[name=name_user]');
            var descricao = $('input[name=description_user');
            var email = $('input[name=email_user]');
            var senha = $('input[name=senha_usuario]');
            var imagem = $('input[name=image_user]');
            var extension = $('input[name=image_user]').val().split('.').pop().toLowerCase();
            ...
            if (senha.val() === '') {
                senha.parent().parent().addClass('has-error');
            } else {
                senha.parent().parent().removeClass('has-error');
                result += '4';
            }
                ...
                $('#myForm').ajaxSubmit({
                    ...
                    success: function(response) 
                    {
                        if (response.success) {
                            ...
                            sucesso('Usuário ' + type + ' com sucesso', '');
                            showAllUsers();
                        } else {
        ...
        $('#showdata').on('click', '.item-edit', function() 
        {
            ...
            $('#myForm').attr('action', '<?php echo base_url() ?>private/UsersController/update');
            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>private/UsersController/edit',
                data: {
                    id_user: id_user
                },
                async: false,
                ...
                success: function(data) 
                {
                    ...
                    $('input[name=description_user]').val(data.description_user);
                    $('input[name=email_user]').val(data.email_user);
                    $('input[name=password_user]').val(data.password_user);
                    $('.aqui').attr("src", "<?php echo base_url('assets/uploads/users/'); ?>" + data.image_user);
        ...
        $('input[name=imagem_usuario]').on('change', function() 
        {
        ...
        $('#showdata').on('click', '.item-delete', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>private/UsersController/delete',
                    data: {
                        id_user: id_user
                    },
                    dataType: 'json',
                    success: function(response) 
                    {
                        if (response.success) {
                            ...
                            info('Sucesso ao deletar usuario', 'Usuario deletado!');
                            showAllUsers();
                        } else {
                            erro('Erro', 'Não foi possivel deletar o usuario.');
                            $('#deleteModal').modal('hide');
                        }
                    },
                    error: function() 
        ...
        $('#myForm2').on('click', '.jq_btn_excluir_marcados', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'post',
                    async: false,
                    url: '<?php echo base_url() ?>private/UsersController/deleteSelected',
                    data: data,
                    dataType: 'json',
                    success: function(response) 
                    {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao deletar', 'Usuários deletados!');
                            showAllUsers();
                        } else {
        ...
        function showAllUsers() 
        {
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>private/UsersController/getAll',
                async: false,
                dataType: 'json',
                success: function(data) 
                {
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                            '<td class="center">' +
                            '<label class="pos-rel">' +
                            '<input name="excluir_todos[]" value="' + data[i].id_user + '" type="checkbox" class="ace" />' +
                            '<span class="lbl"></span>' +
                            '</label>' +
                            '</td>' +
                            '<td>' + data[i].id_user + '</td>' +
                            '<td style="width: 20%">' +
                            '<img src="<?php echo base_url('assets/uploads/users/'); ?>' + data[i].image_user + '" class="img-responsive" alt="User Image" width="250" height="">' +
                            '</td>' +
                            '<td>' + data[i].name_user + '</td>' +
                            '<td>' + data[i].email_user + '</td>' +
                            '<td> <input disabled type="password" value="' + data[i].password_user + '" /></td>' +
                            ...
                            '<i class="ace-icon fa fa-pencil bigger-130 item-edit" data="' + data[i].id_user + '" title="Atualizar"></i>' +
                            '</a>' +
                            '<a class="red" href="javascript:;" >' +
                            '<i class="ace-icon fa fa-trash-o bigger-130 item-delete" data="' + data[i].id_user + '" title="Remover"></i>' +
                            '</a>' +
                            '</div>' +
                            '</td>' +
                            '</tr>';
