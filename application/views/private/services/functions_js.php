...
<script>
    $(function () {
        ...
        $('#btnAdd').click(function () {
            ...
            $('#myForm').attr('action', '<?php echo base_url() ?>private/ServicesController/add');
        ...
        $('#btnSave').click(function () {
            // ENCAPSULANDO INPUTS
            var titulo      = $('input[name=title_service]');
            var texto       = $('input[name=text_service]');
            var link        = $('input[name=link_service]');
            var imagem      = $('input[name=image_service]');
            var extension   = $('input[name=image_service]').val().split('.').pop().toLowerCase();
                ...
                $('#myForm').ajaxSubmit({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            sucesso('Service ' + type + ' com sucesso', '');
                            showAllServices();
                        } else {
        ...
        $('#showdata').on('click', '.item-edit', function () {
            var id_service = $(this).attr('data');
            $('#myModal').modal('show');
            $('#img_atual').fadeIn('fast');
            $('#myModal').find('.modal-title').text('Editar');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/ServicesController/update');
            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>private/ServicesController/getById',
                data: {id_service: id_service},
                async: false,
                dataType: 'json',
                success: function (data) {
                    $('input[name=id_service]').val(data.id_service);
                    $('input[name=title_service]').val(data.title_service);
                    $('input[name=text_service]').val(data.text_service);
                    $('input[name=link_service]').val(data.link_service);
                    $('.aqui').attr("src", "<?php echo base_url('assets/uploads/services/'); ?>" + data.image_service);
                    if (data.status == '1') {
                        $('.ios-switch').removeClass('off').addClass('on').next('input:checkbox').attr('checked', true).val('1');
                        $('.stt').html('Ativo');
                    } else {
        ...
        $('input[name=image_service]').on('change', function ()
        {
        ...
        $('#showdata').on('click', '.item-delete', function () {
            ...
            $('#btnDelete').unbind().click(function () {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>private/ServicesController/delete',
                    data: {id_service: id_service},
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao deletar serviços', 'Serviço excluído!');
                            showAllServices();
                        } else {
        ...
        $('#myForm2').on('click', '.jq_btn_excluir_marcados', function () {
            ...
            $('#btnDelete').unbind().click(function () {
                $.ajax({
                    ...
                    method: 'post',
                    async: false,
                    url: '<?php echo base_url() ?>private/ServicesController/deleteSelected',
                    data: data,
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao deletar', 'Serviços excluídos!');
                            showAllServices();
                        } else {
        ...
        function showAllServices() {
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>private/ServicesController/getAll',
                async: false,
                dataType: 'json',
                success: function (data) {
                    var html = '';
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                                '<td class="center">' +
                                '<label class="pos-rel">' +
                                '<input name="excluir_todos[]" value="' + data[i].id_service + '" type="checkbox" class="ace" />' +
                                '<span class="lbl"></span>' +
                                '</label>' +
                                '</td>' +
                                '<td>' + data[i].id_service + '</td>' +
                                '<td style="width: 20%">' +
                                '<img src="<?php echo base_url('assets/uploads/services/'); ?>' + data[i].image_service + '" class="img-responsive" alt="User Image" width="250" height="">' +
                                '</td>' +
                                '<td>' + data[i].title_service + '</td>' +
                                '<td>' + data[i].text_service + '</td>' +
                                ...
                                '<i class="ace-icon fa fa-pencil bigger-130 item-edit" data="' + data[i].id_service + '" title="Atualizar"></i>' +
                                '</a>' +
                                '<a class="red" href="javascript:;" >' +
                                '<i class="ace-icon fa fa-trash-o bigger-130 item-delete" data="' + data[i].id_service + '" title="Remover"></i>' +
                                '</a>' +
                                '</div>' +
                                '</td>' +
                                '</tr>';
