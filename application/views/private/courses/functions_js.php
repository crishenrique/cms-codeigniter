...
<script>
    $(function() 
    {
        ...
        $('#btnAdd').click(function() 
        {
            ...
            $('#myForm').attr('action', '<?php echo base_url() ?>private/CoursesController/add');
        ...
        {
            //ENCAPSULANDO INPUTS
            var title = $('input[name=title_course]');
            var text = $('input[name=text_course]');
            var price = $('input[name=price_course]');
            var image = $('input[name=image_course]');
            var extension = $('input[name=image_course]').val().split('.').pop().toLowerCase();
        ...
        {
            var id_course = $(this).attr('data');
            $('#myModal').modal('show');
            $('#img_atual').fadeIn('fast');
            $('#myModal').find('.modal-title').text('Editar curso');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/CoursesController/update');
            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>private/CoursesController/getById',
                data: {
                    id_course: id_course
                },
                async: false,
                ...
                {
                    $('input[name=title_course]').val(data.title_course);
                    $('input[name=text_course]').val(data.text_course);

                    $('#summernote').summernote('code', data.text_course);

                    $('input[name=price_course]').val(data.price_course);
                    $('.aqui').attr("src", "<?php echo base_url('assets/uploads/courses/'); ?>" + data.image_course);
        ...
        $('input[name=image_course]').on('change', function() 
        {
        ...
        $('#showdata').on('click', '.item-delete', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>private/CoursesController/delete',
                    data: {
                        id_course: id_course
                    },
                    dataType: 'json',
        ...
        $('#myForm2').on('click', '.jq_btn_excluir_marcados', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'post',
                    async: false,
                    url: '<?php echo base_url() ?>private/CoursesController/deleteSelected',
                    data: data,
                    dataType: 'json',
                    success: function(response) 
                    {
        ...
        function showAllCourses() 
        {
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>private/CoursesController/getAll',
                async: false,
                dataType: 'json',
                success: function(data) 
                {
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                            '<td class="center">' +
                            '<label class="pos-rel">' +
                            '<input name="excluir_todos[]" value="' + data[i].id_course + '" type="checkbox" class="ace" />' +
                            '<span class="lbl"></span>' +
                            '</label>' +
                            '</td>' +
                            '<td>' + data[i].id_course + '</td>' +
                            '<td style="width: 20%">' +
                            '<img src="<?php echo base_url('assets/uploads/courses/'); ?>' + data[i].image_course + '" class="img-responsive" alt="User Image" width="250" height="">' +
                            '</td>' +
                            '<td>' + data[i].title_course + '</td>' +
                            '<td>' + data[i].text_course + '</td>' +
                            '<td>' + data[i].price_course + '</td>' +
                            ...
                            '<i class="ace-icon fa fa-pencil bigger-130 item-edit" data="' + data[i].id_course + '" title="Atualizar"></i>' +
                            '</a>' +
                            '<a class="red" href="javascript:;" >' +
                            '<i class="ace-icon fa fa-trash-o bigger-130 item-delete" data="' + data[i].id_course + '" title="Remover"></i>' +
                            '</a>' +
                            '</div>' +
                            '</td>' +
                            '</tr>';
