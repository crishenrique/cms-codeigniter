...
<script>
    $(function() 
    {
        ...
        $('#btnAdd').click(function() 
        {
            ...
            $('#myForm').attr('action', '<?php echo base_url() ?>private/PartnersController/add');
        ...
        {
            //ENCAPSULANDO INPUTS
            var title = $('input[name=title_partner]');
            var text = $('input[name=text_partner]');
            var price = $('input[name=price_partner]');
            var image = $('input[name=image_partner]');
            var extension = $('input[name=image_partner]').val().split('.').pop().toLowerCase();
        ...
        {
            var id_partner = $(this).attr('data');
            $('#myModal').modal('show');
            $('#img_atual').fadeIn('fast');
            $('#myModal').find('.modal-title').text('Editar curso');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/PartnersController/update');
            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>private/PartnersController/getById',
                data: {
                ...
                {
                    $('input[name=title_partner]').val(data.title_partner);
                    $('input[name=text_partner]').val(data.text_partner);

                    $('#summernote').summernote('code', data.text_partner);

                    $('input[name=price_partner]').val(data.price_partner);
                    $('.aqui').attr("src", "<?php echo base_url('assets/uploads/partners/'); ?>" + data.image_partner);
                    $('input[name=id_partner]').val(data.id_partner);
        ...
        $('input[name=image_partner]').on('change', function() 
        {
        ...
        $('#showdata').on('click', '.item-delete', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>private/PartnersController/delete',
                    data: {
                        id_partner: id_partner
                    },
                    dataType: 'json',
        ...
        $('#myForm2').on('click', '.jq_btn_excluir_marcados', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'post',
                    async: false,
                    url: '<?php echo base_url() ?>private/PartnersController/deleteSelected',
                    data: data,
                    dataType: 'json',
                    success: function(response) 
                    {
        ...
        function showAllPartners() 
        {
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>private/PartnersController/getAll',
                async: false,
                dataType: 'json',
                success: function(data) 
                {
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                            '<td class="center">' +
                            '<label class="pos-rel">' +
                            '<input name="excluir_todos[]" value="' + data[i].id_partner + '" type="checkbox" class="ace" />' +
                            '<span class="lbl"></span>' +
                            '</label>' +
                            '</td>' +
                            '<td>' + data[i].id_partner + '</td>' +
                            '<td style="width: 20%">' +
                            '<img src="<?php echo base_url('assets/uploads/partners/'); ?>' + data[i].image_partner + '" class="img-responsive" alt="User Image" width="250" height="">' +
                            '</td>' +
                            '<td>' + data[i].title_partner + '</td>' +
                            '<td>' + data[i].text_partner + '</td>' +
                            '<td>' + data[i].price_partner + '</td>' +
                            ...
                            '<i class="ace-icon fa fa-pencil bigger-130 item-edit" data="' + data[i].id_partner + '" title="Atualizar"></i>' +
                            '</a>' +
                            '<a class="red" href="javascript:;" >' +
                            '<i class="ace-icon fa fa-trash-o bigger-130 item-delete" data="' + data[i].id_partner + '" title="Remover"></i>' +
                            '</a>' +
                            '</div>' +
                            '</td>' +
                            '</tr>';
