...
<script>
    $(function() 
    {
        ...
        $('#btnAdd').click(function() 
        {
            $('#myModal').modal('show');
            $('#img_atual').hide();
            $('#myModal').find('.modal-title').text('Adicionar Novo Banner');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/BannersController/add');
        ...
        {
            // ENCAPSULANDO INPUTS
            var titulo = $('input[name=title_banner]');
            // var texto = $('input[name=text_banner]');
            var link = $('input[name=link_banner]');
            var imagem = $('input[name=image_banner]');
            var extension = $('input[name=image_banner]').val().split('.').pop().toLowerCase();
                ...
                $('#myForm').ajaxSubmit({
                    ...
                    success: function(response) 
                    {
                        if (response.success) {
                            ...
                            sucesso('Banner ' + type + ' com sucesso', '');
                            showAllBanners();
                        } else {
        ...
        {
            var id_banner = $(this).attr('data');

            $('#myModal').modal('show');
            $('#img_atual').fadeIn('fast');
            $('#myModal').find('.modal-title').text('Editar');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/BannersController/update');

            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>private/BannersController/getById',
                data: {
                    id_banner: id_banner
                },
                async: false,
                ...
                success: function(data) 
                {
                    ...
                    $('input[name=title_banner]').val(data.title_banner);
                    // $('input[name=text_banner]').val(data.text_banner);
                    $('#summernote').summernote('code', data.text_banner);
                    $('input[name=link_banner]').val(data.link_banner);
                    $('.aqui').attr("src", "<?php echo base_url('assets/uploads/banners/'); ?>" + data.image_banner);
                    if (data.status == '1') {
                        $('.ios-switch').removeClass('off').addClass('on').next('input:checkbox').attr('checked', true).val('1');
                        $('.stt').html('Ativo');
                    } else {
        ...
        $('#showdata').on('click', '.item-delete', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>private/BannersController/delete',
                    data: {
                        id_banner: id_banner
                    },
                    dataType: 'json',
                    success: function(response) 
                    {
                        if (response.success) {
                            ...
                            info('Sucesso ao deletar banner', 'Banner deletado!');
                            showAllBanners();
                        } else {
                            erro('Erro', 'Não foi possivel deletar o banner.');
                            $('#deleteModal').modal('hide');
                        }
                    },
                    error: function() 
        ...
        $('#myForm2').on('click', '.jq_btn_excluir_marcados', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'post',
                    async: false,
                    url: '<?php echo base_url() ?>private/BannersController/deleteSelected',
                    data: data,
                    dataType: 'json',
                    success: function(response) 
                    {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao deletar', 'Banners deletados!');
                            showAllBanners();
                        } else {
        ...
        function showAllBanners() 
        {
            ...
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>private/BannersController/getAll',
                async: false,
                dataType: 'json',
                success: function(data)
                 {
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                            '<td class="center">' +
                            '<label class="pos-rel">' +
                            '<input name="excluir_todos[]" value="' + data[i].id_banner + '" type="checkbox" class="ace" />' +
                            '<span class="lbl"></span>' +
                            '</label>' +
                            '</td>' +
                            '<td>' + data[i].id_banner + '</td>' +
                            '<td style="width: 20%">' +
                            '<img src="<?php echo base_url('assets/uploads/banners/'); ?>' + data[i].image_banner + '" class="img-responsive" alt="User Image" width="250" height="">' +
                            '</td>' +
                            '<td>' + data[i].title_banner + '</td>' +
                            // '<td>' + data[i].text_banner + '</td>' +
                            '<td>' + data[i].link_banner + '</td>' +
                            ...
                            '<i class="ace-icon fa fa-pencil bigger-130 item-edit" data="' + data[i].id_banner + '" title="Atualizar"></i>' +
                            '</a>' +
                            '<a class="red" href="javascript:;" >' +
                            '<i class="ace-icon fa fa-trash-o bigger-130 item-delete" data="' + data[i].id_banner + '" title="Remover"></i>' +
                            '</a>' +
                            '</div>' +
                            '</td>' +
                            '</tr>';
