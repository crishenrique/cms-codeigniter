...
<script>
    $(function() 
    {
        ...
        $('#btnAdd').click(function() 
        {
            ...
            $('#myForm').attr('action', '<?php echo base_url() ?>private/NewsController/add');
        ...
        {
            /* ENCAPSULANDO INPUTS */
            var titulo = $('input[name=title_news]');
            var resumo = $('input[name=abstract_news]');

            var texto = $('input[name=text_news]');
            //PEGA OS VALORES EXATOS DO EDITOR
            var markupStr = $('#summernote').summernote('code');
            //SETA NO VALOR DA VARIÁVEL TEXTO
            texto.val(markupStr);

            var fonte = $('input[name=origin_news');
            var autor = $('input[name=id_author_news');
            var imagem = $('input[name=image_news]');
            var extension = $('input[name=image_news]').val().split('.').pop().toLowerCase();
            ...
            if (resumo.val() === '') {
                resumo.parent().parent().addClass('has-error');
            } else {
                resumo.parent().parent().removeClass('has-error');
                result += '2';
            }
            if (texto.val() === '') {
                texto.parent().parent().addClass('has-error');
            } else {
                texto.parent().parent().removeClass('has-error');
                result += '3';
            }

            if (fonte.val() === '') {
                fonte.parent().parent().addClass('has-error');
            } else {
                fonte.parent().parent().removeClass('has-error');
                result += '4';
            }
            if (autor.val() === '') {
                autor.parent().parent().addClass('has-error');
            } else {
                autor.parent().parent().removeClass('has-error');
                result += '5';
            }
                ...
                $('#myForm').ajaxSubmit({
                    ...
                    success: function(response) 
                    {
                        if (response.success) {
                            ...
                            sucesso('Notícia ' + type + ' com sucesso', '');
                            showAllNews();
                        } else {
                            erro('Não foi possível add notícia!', 'Erro Cod: 002');
                            $('#myModal').modal('hide');
                        }
                    },
                    error: function() 
        ...
        {
            var id_news = $(this).attr('data');
            $('#myModal').modal('show');
            $('#img_atual').fadeIn('fast');
            $('#myModal').find('.modal-title').text('Editar');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/NewsController/update');
            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>private/NewsController/getById',
                data: {
                    id_news: id_news
                },
                async: false,
                ...
                success: function(data) 
                {
                    ...
                    $('input[name=id_author_news]').val(data.id_author_news);
                    $('input[name=title_news]').val(data.title_news);
                    $('input[name=abstract_news]').val(data.abstract_news);

                    $('input[name=text_news]').val(data.text_news);
                    $('#summernote').summernote('code', data.text_news);

                    $('input[name=origin_news]').val(data.origin_news);

                    $('.aqui').attr("src", "<?php echo base_url('assets/uploads/news/'); ?>" + data.image_news);
        ...
        $('input[name=image_news]').on('change', function() 
        {
        ...
        $('#showdata').on('click', '.item-delete', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>private/NewsController/delete',
                    data: {
                        id_news: id_news
                    },
                    dataType: 'json',
                    success: function(response) 
                    {
                        if (response.success) {
                            ...
                            info('Sucesso ao deletar a notícia', 'Notícia deletada!');
                            showAllNews();
                        } else {
                            erro('Erro', 'Não foi possivel deletar a notícia.');
                            $('#deleteModal').modal('hide');
                        }
                    },
                    error: function() 
        ...
        $('#myForm2').on('click', '.jq_btn_excluir_marcados', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'post',
                    async: false,
                    url: '<?php echo base_url() ?>private/NewsController/deleteSelected',
                    data: data,
                    dataType: 'json',
                    success: function(response) 
                    {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao deletar as notícias selecionadas', 'Notícias deletadas!');
                            showAllNews();
                        } else {
                            atencao('Atenção', 'Escolha no mínimo uma notícia para excluir!');
                            $('#deleteModal').modal('hide');
                        }
                    },
                    error: function() 
        ...
        function showAllNews() 
        {
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>private/NewsController/getAll',
                async: false,
                dataType: 'json',
                success: function(data) 
                {
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                            '<td class="center">' +
                            '<label class="pos-rel">' +
                            '<input name="excluir_todos[]" value="' + data[i].id_news + '" type="checkbox" class="ace" />' +
                            '<span class="lbl"></span>' +
                            '</label>' +
                            '</td>' +
                            '<td>' + data[i].id_news + '</td>' +
                            '<td style="width: 20%">' +
                            '<img src="<?php echo base_url('assets/uploads/news/'); ?>' + data[i].image_news + '" class="img-responsive" alt="User Image" width="250" height="">' +
                            '</td>' +
                            '<td>' + data[i].title_news + '</td>' +
                            '<td>' + data[i].abstract_news + '</td>' +
                            ...
                            '<i class="ace-icon fa fa-pencil bigger-130 item-edit" data="' + data[i].id_news + '" title="Atualizar"></i>' +
                            '</a>' +
                            '<a class="red" href="javascript:;" >' +
                            '<i class="ace-icon fa fa-trash-o bigger-130 item-delete" data="' + data[i].id_news + '" title="Remover"></i>' +
                            '</a>' +
                            '</div>' +
                            '</td>' +
                            '</tr>';
