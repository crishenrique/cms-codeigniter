...
<script>
    $(function() 
    {
        ...
        $('#btnAdd').click(function() 
        {
            $('#myModal').modal('show');
            $('#img_atual').hide();
            $('#myModal').find('.modal-title').text('Adicionar Novo Comentário');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/TestimonialsController/add');
        ...
        {
            // ENCAPSULANDO INPUTS
            var title = $('input[name=name_testimonial]');
            var texto = $('input[name=text_testimonial]');
            var city = $('input[name=city_testimonial]');
            var imagem = $('input[name=image_testimonial]');
            var extension = $('input[name=image_testimonial]').val().split('.').pop().toLowerCase();
            ...
            if (imagem.val() !== '' && jQuery.inArray(extension, ['gif', 'png', 'jpg', 'jpeg']) === -1) {
                ...
                $('input[name=image_testimonial]').val('');
                ...
                $('#myForm').ajaxSubmit({
                    ...
                    success: function(response) 
                    {
                        if (response.success) {
                            ...
                            sucesso('Comentário ' + type + ' com sucesso', '');
                            showAllTestimonials();
                        } else {
        ...
        {
            var id_testimonial = $(this).attr('data');
            $('#myModal').modal('show');
            $('#img_atual').fadeIn('fast');
            $('#myModal').find('.modal-title').text('Editar');
            $('#myForm').attr('action', '<?php echo base_url() ?>private/TestimonialsController/update');
            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>private/TestimonialsController/getById',
                data: {
                ...
                {
                    $('input[name=id_testimonial]').val(data.id_testimonial);
                    $('input[name=name_testimonial]').val(data.name_testimonial);
                    $('input[name=text_testimonial]').val(data.text_testimonial);
                    $('#summernote').summernote('code', data.text_testimonial);
                    $('input[name=city_testimonial]').val(data.city_testimonial);
                    $('.aqui').attr("src", "<?php echo base_url('assets/uploads/testimonials/'); ?>" + data.image_testimonial);
        ...
        $('input[name=image_testimonial]').on('change', function() 
        {
        ...
        $('#showdata').on('click', '.item-delete', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>private/TestimonialsController/delete',
                    data: {
                        id_testimonial: id_testimonial
                    },
                    dataType: 'json',
                    success: function(response) 
                    {
                        if (response.success) {
                            ...
                            info('Sucesso ao deletar Comentário', 'Comentário deletado!');
                            showAllTestimonials();
                        } else {
                            erro('Erro', 'Não foi possivel deletar o Comentário.');
                            $('#deleteModal').modal('hide');
                        }
                    },
                    error: function() 
        ...
        $('#myForm2').on('click', '.jq_btn_excluir_marcados', function() 
        {
            ...
            $('#btnDelete').unbind().click(function() 
            {
                $.ajax({
                    ...
                    method: 'post',
                    async: false,
                    url: '<?php echo base_url() ?>private/TestimonialsController/deleteSelected',
                    data: data,
                    dataType: 'json',
                    success: function(response) 
                    {
                        if (response.success) {
                            $('#deleteModal').modal('hide');
                            info('Sucesso ao deletar', 'Comentários deletados!');
                            showAllTestimonials();
                        } else {
        ...
        function showAllTestimonials() {
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>private/TestimonialsController/getAll',
                async: false,
                dataType: 'json',
                success: function(data) 
                {
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                            '<td class="center">' +
                            '<label class="pos-rel">' +
                            '<input name="excluir_todos[]" value="' + data[i].id_testimonial + '" type="checkbox" class="ace" />' +
                            '<span class="lbl"></span>' +
                            '</label>' +
                            '</td>' +
                            '<td>' + data[i].id_testimonial + '</td>' +
                            '<td style="width: 20%">' +
                            '<img src="<?php echo base_url('assets/uploads/testimonials/'); ?>' + data[i].image_testimonial + '" class="img-responsive" alt="User Image" width="250" height="">' +
                            '</td>' +
                            '<td>' + data[i].name_testimonial + '</td>' +
                            '<td>' + data[i].text_testimonial + '</td>' +
                            '<td>' + data[i].city_testimonial + '</td>' +
                            ...
                            '<i class="ace-icon fa fa-pencil bigger-130 item-edit" data="' + data[i].id_testimonial + '" title="Atualizar"></i>' +
                            '</a>' +
                            '<a class="red" href="javascript:;" >' +
                            '<i class="ace-icon fa fa-trash-o bigger-130 item-delete" data="' + data[i].id_testimonial + '" title="Remover"></i>' +
                            '</a>' +
                            '</div>' +
                            '</td>' +
                            '</tr>';
